# Vendor Risk Recommendation — BrightSlot, Inc.

**To:** Meridian Health Analytics — Procurement & Leadership
**From:** GRC (Callum McRae)
**Date:** 2026-09
**Re:** Security assessment of BrightSlot for the partner-clinic booking widget

---

## Recommendation

**Approve with conditions.** BrightSlot is a **Medium-risk** vendor (weighted score
...
Do **not** sign on BrightSlot's standard terms.
...
1. **BrightSlot uses customer data for its own analytics.** Even "de-identified," Meridian
...
2. **BrightSlot will not sign a BAA.** It argues it never handles PHI. That position is
   debatable — appointment type and clinic, linked over time, can be health-related
   information. If Meridian's counsel disagrees with BrightSlot, Meridian is exposed.
...
| # | Condition | Addresses |
|---|---|---|
| C1 | Execute a Data Processing Agreement with HIPAA-aligned safeguards. Obtain BrightSlot's written legal position on Business Associate status; if Meridian's counsel concludes a BAA is required, a BAA is a precondition to go-live. | BAA gap (B5) |
...
| C3 | Contractual breach notification **within 72 hours** of BrightSlot confirming a security incident affecting Meridian data, with a defined minimum content set. | Breach SLA (E2) |
...
| # | Control | Addresses |
|---|---|---|
| M1 | Scope the BrightSlot API credential to the booking endpoints only; rotate every 90 days; store in Meridian's secrets manager. | Credential compromise |
...
| M3 | Quarterly manual check that departed Meridian staff have no BrightSlot access (no SCIM available). | Deprovisioning gap (D4) |
...
- Logical (not physical) tenant isolation — mitigated by BrightSlot's SOC 2 controls and by C6 data minimisation.
...
- DAST and formal threat modelling absent from BrightSlot's SDLC — pen testing partially compensates.

## Review cadence

- **Reassess annually**, or sooner on any of: a security incident affecting Meridian data;
  a material new subprocessor; a change of control / acquisition of BrightSlot; loss or
